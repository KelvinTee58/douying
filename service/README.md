...
| `name`               | `STRING(100)`         | 姓名                         | 0.0.1           | 是               |
| `gender`             | `ENUM('M', 'F', 'O')` | [性别类型](#employee_gender) | 0.0.1           |                  |
...
| `detailedAddress`    | `VARCHAR`             | 详细地址                    | 0.0.1           | 是               |
| `areaCode`           | `STRING(50)`          | 区域代码                    | 0.0.1           | 是               |
| `type`               | `ENUM('P', 'R', 'O')` | [仓库类型](#warehouse_type) | 0.0.1           | 是               |
...
| `productionBatch`    | `STRING(100)`                   | 生产批次编号，用于标识生产批次(unique) | 0.0.1           | 是               |
| `batchSequence`      | `INT`                           | 生产批次号，用于第几批次               | 0.0.1           | 是               |
| `type`               | `ENUM`：[表末](#producing_type) | [生产类型](#producing_type)            | 0.0.1           | 是               |
...
| `productionBatch`        | `STRING(100)`                 | 生产批次（外键，关联 `producing.productionBatch`） | 0.0.1           | 是               |
...
- <a id="inbound_type"></a> `type` 字段的 ENUM 值：`ENUM('IN', 'OUT','COMPLETED', 'WITHDRAWAL'，'O')`
...
| `productionBatch`    | `STRING(100)`                   | 生产批次（外键，关联 `producing.productionBatch`）  | 0.0.1           | 是               |
| `type`               | `ENUM`：[表末](#inventory_type) | [记录类型](#inventory_type)                         | 0.0.1           | 是               |
