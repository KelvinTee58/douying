...
export default class Timeline extends Vue {
  ...
  descRef = [] as HTMLElement[];
  dotsHeight = [] as string[];
  ...
  mounted() {
    this.getDotsHeight();
    this.applyAlternateClasses();
  }

  getDotsHeight() {
    this.dotsHeight = this.descRef.map((el) => {
      const lineHeight = window
        .getComputedStyle(el.firstElementChild || el)
        .getPropertyValue('line-height');
      return lineHeight;
    });
  }

  applyAlternateClasses() {
    if (this.mode === 'center') {
      this.$nextTick(() => {
        this.descRef.forEach((el, index) => {
          if ((index + 1) % 2) {
            // odd
            if (this.position === 'left') {
              el.classList.add('desc-alternate-left');
            } else {
              el.classList.add('desc-alternate-right');
            }
          } else {
            // even
            if (this.position === 'left') {
      ...
      >
        <slot name="dot" :item="item" :index="index">
          ...
          <span
            class="dot-item"
            v-else
            :style="{ borderColor: item.color || '#1677ff' }"
          ></span>
        </slot>
      </div>
